...
def process_rules_list(temp_mapping_path, urls):
    ...
    for url in urls:
        ...
        if url not in existing_urls:
            # 将新URL添加到新条目列表中，其他字段初始化为空
            new_entries.append({'URL': url, 'RemoteFileName': '', 'LocalFileName': '', 'Hash': ''})

    # 如果有新条目需要添加
    if new_entries:
        # 将新条目列表转换为DataFrame对象
        new_df = pd.DataFrame(new_entries)
        # 将新的DataFrame对象与原DataFrame对象拼接，忽略原索引
        df = pd.concat([df, new_df], ignore_index=True)

    # 将更新后的DataFrame对象写入临时映射文件，不包含行索引
...
def generate_unique_local_filenames(temp_mapping_path):
    ...
    for index, row in df.iterrows():
        ...
        if remote_filename in duplicates:
            # 分割URL为路径段列表
            parts = row['URL'].split('/')
            # 获取URL路径的倒数第二个部分作为唯一标识，如果路径段不足2个则为空
